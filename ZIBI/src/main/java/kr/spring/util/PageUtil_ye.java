...
public class PageUtil_ye {
	 ...
	 * */
	public PageUtil_ye(int currentPage,int count, int rowCount) {
	...
	public PageUtil_ye(int currentPage, int count, int rowCount, int pageCount, String pageUrl) {
		this(null,null,currentPage,count,rowCount,pageCount,pageUrl,null);
	}
	public PageUtil_ye(int currentPage, int count, int rowCount, int pageCount, String pageUrl, String addKey) {
		this(null,null,currentPage,count,rowCount,pageCount,pageUrl,addKey);
	}
	public PageUtil_ye(String keyfield, String keyword, int currentPage, int count, int rowCount, int pageCount,String pageUrl) {
		this(keyfield,keyword,currentPage,count,rowCount,pageCount,pageUrl,null);
	}
	public PageUtil_ye(String keyfield, String keyword, int currentPage, int count, int rowCount, int pageCount,String pageUrl,String addKey) {
		...
		if(count >= 0) {
			...
			if(pageCount > 0) {
				...
				for (int i = startPage; i <= endPage; i++) {
					...
					if (i == currentPage) {
						page.append("&nbsp;<b><span style='color:#32a77b; font-size:1.1em;'>");
						page.append(i);
						page.append("</span></b>");
					} else {
				...
				if (totalPage - startPage >= pageCount) {
					page.append("<a href="+pageUrl+"?pageNum="+ (endPage + 1) + sub_url +" style='decoration-text:none; color:balck;'>");
					page.append("[다음]");
					page.append("</a>");
				}
			}else {
