	...
	<c:if test="${count>0}">
		<div class="text-end mypage-category">
			<select id="category" class="selectbox" name="category">
				...
				<option value="3" <c:if test="${param.category==3}">selected</c:if>>재능 기부</option>
				<option value="4" <c:if test="${param.category==4}">selected</c:if>>영화 예매</option>
				<option value="5">중고 거래</option>
			</select>
		</div>
